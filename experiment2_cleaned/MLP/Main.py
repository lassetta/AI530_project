...
def eval_network(net, crit, DL, act):
...
def train_network(DL_train, DL_val,net,crit,opt,act,fs):
  MAX_EPOCHS = 100
  ...
  accuracy_res = np.zeros((MAX_EPOCHS,2))
  ...
  for j in t:
    ...
    train_loss, train_acc = eval_network(net,crit,DL_train,act)
    accuracy_res[j,0] = train_acc
    val_loss, val_acc = eval_network(net,crit,DL_val,act)
  ...
  run_t = np.expand_dims(np.array(running_time),axis = -1)
  np.savetxt(fs+"time.csv", run_t, delimiter = ',')
...
if __name__ == "__main__":
  ...
  # build the model
  activations = [torch.sigmoid, torch.tanh, F.relu, F.leaky_relu]
  fs = ["sig", "tanh", "relu", "leaky"]
  for i in range(4):
    net = gen_model().to(device) 
    crit = torch.nn.CrossEntropyLoss()
    opt = optim.Adam(net.parameters(), lr = 1e-4)
    train_network(DL_train, DL_val,net,crit,opt,activations[i],fs[i])
