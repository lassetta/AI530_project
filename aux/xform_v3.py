...
def transform(img,focus_x, focus_y, mag_rad, mag_d, demag_w):
  ...
  fish_inv[norms <= xc] = f2(norms[norms <= xc])
  idcs2 = np.logical_and(norms > xc, norms < 1)
  fish_inv[idcs2] = f1(norms[idcs2])
...
if __name__ == "__main__":
  ...
  for fold in os.listdir(ROOT):
    ...
    for f in os.listdir(class_root):
      ...
      plt.imshow(img)
      plt.show()
      img2 = transform(img,img.shape[0]/2,img.shape[1]/2,min(img.shape[0]//2, img.shape[1]//2),np.random.uniform(1,2),.1)
      plt.imshow(img2)
      plt.show()
